...
async function unlikeVideo(videoStatsId: string, profileCapId: string) {
  const tx = new TransactionBlock();

  tx.moveCall({
    arguments: [tx.object(videoStatsId), tx.object(profileCapId)],
    target: `${getId("package")}::video::unlike`,
  });
...
(async () => {
  ...
  await unlikeVideo(videStatsId, profileCapId);
